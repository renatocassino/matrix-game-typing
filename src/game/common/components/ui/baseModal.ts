...
export class BaseModal extends Phaser.Scene {
  ...
  create() {
    ...
    const closeButton = new IconButton(this, cardSize.width - 30, 30, assets.icon.CLOSE).on(
      'pointerdown',
      () => {
        this.scene.stop();
      }
    ...
    const container = this.add
      .container(cardSize.width, cardSize.height)
      .setPosition(boardWidth / 2 - cardSize.width / 2, boardHeight / 2 - cardSize.height / 2);

    const title = this.add
      .text(xCenter, 35, this.title ?? '', { font: '18px Orbitron', color: '#fff' })
      .setOrigin(0.5, 0.5);

    const disclaimer = this.add
      .text(xCenter, cardSize.height - 40, 'Press ESC to close', {
        font: '14px Orbitron',
        color: '#fff'
      })
      .setOrigin(0.5, 0.5);
