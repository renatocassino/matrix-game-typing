import { LetterStatus, type WordStatus } from '../../../types';
...
export class WordComponent extends Phaser.GameObjects.Container {
  ...
  keyNextLetter() {
    ...
    ['n0', 'n1'].forEach((n) => {
      this.add(
        this.scene.add.particles(0, this.indexTyped * this.letterSize, n, {
          speed: 100,
          gravityY: 0,
          quantity: 2,
    ...
    if (this.indexTyped === this.word.length) {
      this.scene.sound.play(assets.audio.EXPLOSION_SMALL, {
        volume: settings.getConfig('fxVolume')
      });
  ...
  shouldRemove() {
    ...
    const disappeared = this.y > this.velocityConfig.finalY;
